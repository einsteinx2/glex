...
#define NSNDMEMDEBUG 0

/* A single block of SPU RAM */
typedef struct nsnd_block_str {
    /* Our queue entry */
    TAILQ_ENTRY(nsnd_block_str)  qent;
...
static TAILQ_HEAD(nsnd_block_q, nsnd_block_str) pool = {0};

/* Reinitialize the pool with the given RAM base offset */
int nsnd_mem_init(uint32 reserve) {
    nsnd_block_t *blk;
    ...
    blk = (nsnd_block_t *)malloc(sizeof(nsnd_block_t));
    memset(blk, 0, sizeof(nsnd_block_t));
    blk->addr = reserve;
...
#if NSNDMEMDEBUG
    dbglog(DBG_DEBUG, "nsnd_mem_init: %d bytes available\n", blk->size);
#endif
...
void nsnd_mem_shutdown() {
    ...
    while(e) {
        ...
        if(e->inuse)
            dbglog(DBG_DEBUG, "nsnd_mem_shutdown: in-use block at %08lx (size %d)\n", e->addr, e->size);
        else
            dbglog(DBG_DEBUG, "nsnd_mem_shutdown: unused block at %08lx (size %d)\n", e->addr, e->size);
...
uint32 nsnd_mem_malloc(size_t size) {
    nsnd_block_t *e, *best = NULL;
    size_t best_size = 4 * 1024 * 1024;

    assert_msg(initted, "Use of nsnd_mem_malloc before nsnd_mem_init");
    ...
    if(best == NULL) {
        dbglog(DBG_ERROR, "nsnd_mem_malloc: no chunks big enough for alloc(%d)\n", size);
        return 0;
    }
    ...
    if(best->size == size) {
#if NSNDMEMDEBUG
        dbglog(DBG_DEBUG, "nsnd_mem_malloc: allocating perfect-fit at %08lx for size %d\n", best->addr, best->size);
#endif
        best->inuse = 1;
        return best->addr;
    }
    ...
    e = (nsnd_block_t*)malloc(sizeof(nsnd_block_t));
    memset(e, 0, sizeof(nsnd_block_t));
...
#if NSNDMEMDEBUG
    dbglog(DBG_DEBUG, "nsnd_mem_malloc: allocating block %08lx for size %d, and leaving %d at %08lx\n",
...
/* Free a chunk of SPU RAM; pointer is expected to be an offset into
   SPU RAM. */
void nsnd_mem_free(uint32 addr) {
    nsnd_block_t *e, *o;

    assert_msg(initted, "Use of nsnd_mem_free before nsnd_mem_init");
    ...
    if(!e) {
        dbglog(DBG_ERROR, "nsnd_mem_free: attempt to free non-existant block at %08lx\n", (uint32)e);
        return;
    }
...
#if NSNDMEMDEBUG
    dbglog(DBG_DEBUG, "nsnd_mem_free: freeing block at %08lx\n", e->addr);
#endif

    /* Can we coalesce with the block before us? */
    o = TAILQ_PREV(e, nsnd_block_q, qent);
...
uint32 nsnd_mem_available() {
    nsnd_block_t *e;
    size_t      largest = 0;

    assert_msg(initted, "Use of nsnd_mem_available before nsnd_mem_init");
