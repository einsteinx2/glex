...
typedef struct strchan {
    ...
    nsnd_stream_callback_t get_data;
...
// Our stream structs
static strchan_t streams[NSTREAM_MAX];

// Separation buffers (for stereo)
int16 * sep_buffer[2] = { NULL, NULL };

/* the address of the sound ram from the SH4 side */
#define NSND_SPU_RAM_BASE            0xa0800000

// Check an incoming handle
#define NCHECK_HND(x) do { \
        assert( (x) >= 0 && (x) < NSTREAM_MAX ); \
        assert( streams[(x)].initted ); \
    } while(0)

/* Set "get data" callback */
void nsnd_stream_set_callback(nsnd_stream_hnd_t hnd, nsnd_stream_callback_t cb) {
    NCHECK_HND(hnd);
    streams[hnd].get_data = cb;
}

void nsnd_stream_filter_add(nsnd_stream_hnd_t hnd, nsnd_stream_filter_t filtfunc, void * obj) {
    filter_t * f;
...
void nsnd_stream_filter_remove(nsnd_stream_hnd_t hnd, nsnd_stream_filter_t filtfunc, void * obj) {
    filter_t * f;
...
static void process_filters(nsnd_stream_hnd_t hnd, void **buffer, int *samplecnt) {
    filter_t * f;
...
void nsnd_stream_prefill(nsnd_stream_hnd_t hnd) {
    void *buf;
    int got;
...
int nsnd_stream_init() {
    /* Create stereo seperation buffers */
    if(!sep_buffer[0]) {
        sep_buffer[0] = memalign(32, (NSND_STREAM_BUFFER_MAX / 2));
        sep_buffer[1] = memalign(32, (NSND_STREAM_BUFFER_MAX / 2));
    }

    /* Finish loading the stream driver */
    if(nsnd_init() < 0) {
        dbglog(DBG_ERROR, "nsnd_stream_init(): nsnd_init() failed, giving up\n");
        return -1;
    }
...
nsnd_stream_hnd_t nsnd_stream_alloc(nsnd_stream_callback_t cb, int bufsize) {
    int i, old;
    nsnd_stream_hnd_t hnd;

    // Get an unused handle
    hnd = -1;
    old = irq_disable();

    for(i = 0; i < NSTREAM_MAX; i++) {
    ...
    /* Setup the callback */
    nsnd_stream_set_callback(hnd, cb);

    /* Initialize our filter chain list */
    TAILQ_INIT(&streams[hnd].filters);

    // Allocate stream buffers
    streams[hnd].spu_ram_sch[0] = nsnd_mem_malloc(streams[hnd].buffer_size * 2);
    streams[hnd].spu_ram_sch[1] = streams[hnd].spu_ram_sch[0] + streams[hnd].buffer_size;

    // And channels
    streams[hnd].ch[0] = nsnd_sfx_chn_alloc();
    streams[hnd].ch[1] = nsnd_sfx_chn_alloc();
    printf("nsnd_stream: alloc'd channels %d/%d\n", streams[hnd].ch[0], streams[hnd].ch[1]);

    return hnd;
}

int nsnd_stream_reinit(nsnd_stream_hnd_t hnd, nsnd_stream_callback_t cb) {
    NCHECK_HND(hnd);
...
void nsnd_stream_destroy(nsnd_stream_hnd_t hnd) {
    filter_t * c, * n;
    ...
    nsnd_sfx_chn_free(streams[hnd].ch[0]);
    nsnd_sfx_chn_free(streams[hnd].ch[1]);
    ...
    nsnd_stream_stop(hnd);
    nsnd_mem_free(streams[hnd].spu_ram_sch[0]);
    memset(streams + hnd, 0, sizeof(streams[0]));
}

/* Shut everything down and free mem */
void nsnd_stream_shutdown() {
    /* Stop and destroy all active stream */
    int i;

    for(i = 0; i < NSTREAM_MAX; i++) {
        if(streams[i].initted)
            nsnd_stream_destroy(i);
    }
...
void nsnd_stream_queue_enable(nsnd_stream_hnd_t hnd) {
    NCHECK_HND(hnd);
    streams[hnd].queueing = 1;
}

void nsnd_stream_queue_disable(nsnd_stream_hnd_t hnd) {
    NCHECK_HND(hnd);
    streams[hnd].queueing = 0;
}

/* Start streaming (or if queueing is enabled, just get ready) */
void nsnd_stream_start(nsnd_stream_hnd_t hnd, uint32 freq, int st) {
    ...
    nsnd_sh4_to_aica_stop();

    /* Prefill buffers */
    nsnd_stream_prefill(hnd);
    ...
    if(!streams[hnd].queueing)
        nsnd_sh4_to_aica_start();
}

/* Actually make it go (in queued mode) */
void nsnd_stream_queue_go(nsnd_stream_hnd_t hnd) {
    NCHECK_HND(hnd);
    nsnd_sh4_to_aica_start();
}

/* Stop streaming */
void nsnd_stream_stop(nsnd_stream_hnd_t hnd) {
    AICA_CMDSTR_CHANNEL(tmp, cmd, chan);
    ...
    /* Channel 1 */
    cmd->cmd_id = streams[hnd].ch[1];
    nsnd_sh4_to_aica(tmp, AICA_CMDSTR_CHANNEL_SIZE);
}
...
int nsnd_stream_poll(nsnd_stream_hnd_t hnd) {
    ...
    /* Get "real" buffer */
    ch0pos = g2_read_32(NSND_SPU_RAM_BASE + AICA_CHANNEL(streams[hnd].ch[0]) + offsetof(aica_channel_t, pos));
    ch1pos = g2_read_32(NSND_SPU_RAM_BASE + AICA_CHANNEL(streams[hnd].ch[1]) + offsetof(aica_channel_t, pos));

    if(ch0pos >= (streams[hnd].buffer_size / 2)) {
        dbglog(DBG_ERROR, "nsnd_stream_poll: chan0(%d).pos = %ld (%08lx)\n", streams[hnd].ch[0], ch0pos, ch0pos);
        return -1;
    }
...
void nsnd_stream_volume(nsnd_stream_hnd_t hnd, int vol) {
    AICA_CMDSTR_CHANNEL(tmp, cmd, chan);
