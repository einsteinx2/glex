...
struct nsnd_effect;
LIST_HEAD(selist, nsnd_effect);

typedef struct nsnd_effect {
    uint32  locl, locr;
    uint32  len;
    uint32  rate;
    uint32  used;
    ...
    LIST_ENTRY(nsnd_effect)  list;
} nsnd_effect_t;

struct selist nsnd_effects;
...
void nsnd_sfx_unload_all() {
    nsnd_effect_t * t, * n;

    t = LIST_FIRST(&nsnd_effects);

    while(t) {
        n = LIST_NEXT(t, list);

        nsnd_mem_free(t->locl);

        if(t->stereo)
            nsnd_mem_free(t->locr);
    ...
    LIST_INIT(&nsnd_effects);
}

/* Unload a single sample */
void nsnd_sfx_unload(sfxhnd_t idx) {
    nsnd_effect_t * t = (nsnd_effect_t *)idx;
    
    if(idx == NSFXHND_INVALID) {
        dbglog(DBG_WARNING, "nsnd_sfx: can't unload an invalid SFXHND\n");
        return;
    }
...
sfxhnd_t nsnd_sfx_load(const char *fn) {
    file_t  fd;
    uint32  len, hz;
    ...
    dbglog(DBG_DEBUG, "nsnd_sfx: loading effect %s\n", fn);

    fd = fs_open(fn, O_RDONLY);

    if(fd <= FILEHND_INVALID) {
        dbglog(DBG_WARNING, "nsnd_sfx: can't open sfx %s\n", fn);
        return NSFXHND_INVALID;
    }
    ...
    if(strncmp((char*)&hz, "WAVE", 4)) {
        dbglog(DBG_WARNING, "nsnd_sfx: file is not RIFF WAVE\n");
        fs_close(fd);
        return NSFXHND_INVALID;
    }
    ...
    t = malloc(sizeof(nsnd_effect_t));
    memset(t, 0, sizeof(nsnd_effect_t));
    ...
    if(stereo == 1) {
        ...
        t->rate = hz;
        t->locl = nsnd_mem_malloc(len / 2);
        t->locr = nsnd_mem_malloc(len / 2);
    ...
    if(t != NSFXHND_INVALID) 
        LIST_INSERT_HEAD(&nsnd_effects, t, list);

    return (sfxhnd_t)t;
}

int nsnd_sfx_play_chn(int chn, sfxhnd_t idx, int vol, int pan) {
    int size;
    nsnd_effect_t * t = (nsnd_effect_t *)idx;
...
int nsnd_sfx_play(sfxhnd_t idx, int vol, int pan) {
    int chn, moved, old;
    ...
    if(moved && chn == sfx_nextchan) {
        ...
        return nsnd_sfx_play_chn(chn, idx, vol, pan);
    }
}

void nsnd_sfx_stop(int chn) {
    AICA_CMDSTR_CHANNEL(tmp, cmd, chan);
    cmd->cmd = AICA_CMD_CHAN;
    cmd->timestamp = 0;
    cmd->size = AICA_CMDSTR_CHANNEL_SIZE;
...
void nsnd_sfx_chn_free(int chn) {
    int old;
