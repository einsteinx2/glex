...
extern uint8 nsnd_stream_drv[];
extern uint8 nsnd_stream_drv_end[];
...
int nsnd_init() {
    ...
    if(!initted) {
        ...
        amt = nsnd_stream_drv_end - nsnd_stream_drv;

        if(amt % 4)
            amt = (amt + 4) & ~3;

        printf("nsnd_init(): loading %d bytes into SPU RAM\n", amt);
        spu_memload(0, nsnd_stream_drv, amt);
...
int nsnd_sh4_to_aica(void *packet, uint32 size) {
    ...
    qa = NSND_SPU_RAM_BASE + AICA_MEM_CMD_QUEUE;
    assert_msg(g2_read_32(qa + offsetof(aica_queue_t, valid)), "Queue is not yet valid");

    bot = NSND_SPU_RAM_BASE + g2_read_32(qa + offsetof(aica_queue_t, data));
...
void nsnd_sh4_to_aica_start() {
    g2_write_32(NSND_SPU_RAM_BASE + AICA_MEM_CMD_QUEUE + offsetof(aica_queue_t, process_ok), 1);
}

/* Stop processing requests in the queue */
void nsnd_sh4_to_aica_stop() {
    g2_write_32(NSND_SPU_RAM_BASE + AICA_MEM_CMD_QUEUE + offsetof(aica_queue_t, process_ok), 0);
}
...
int nsnd_aica_to_sh4(void *packetout) {
    uint32  bot, start, stop, top, size, cnt, *pkt32;

    sem_wait(&sem_qram);

    /* Set these up for reference */
    bot = NSND_SPU_RAM_BASE + AICA_MEM_RESP_QUEUE;
    assert_msg(g2_read_32(bot + offsetof(aica_queue_t, valid)), "Queue is not yet valid");

    top = NSND_SPU_RAM_BASE + AICA_MEM_RESP_QUEUE + g2_read_32(bot + offsetof(aica_queue_t, size));
    start = NSND_SPU_RAM_BASE + AICA_MEM_RESP_QUEUE + g2_read_32(bot + offsetof(aica_queue_t, tail));
    stop = NSND_SPU_RAM_BASE + AICA_MEM_RESP_QUEUE + g2_read_32(bot + offsetof(aica_queue_t, head));
    cnt = 0;
    ...
    if(cnt >= AICA_CMD_MAX_SIZE) {
        sem_signal(&sem_qram);
        dbglog(DBG_ERROR, "nsnd_aica_to_sh4(): packet larger than %d dwords\n", AICA_CMD_MAX_SIZE);
        return -1;
    }
    ...
    if(stop > top)
        stop -= top - (NSND_SPU_RAM_BASE + AICA_MEM_RESP_QUEUE);
    ...
    g2_write_32(bot + offsetof(aica_queue_t, tail), start - (NSND_SPU_RAM_BASE + AICA_MEM_RESP_QUEUE));
...
void nsnd_poll_resp() {
    ...
    while((rv = nsnd_aica_to_sh4(pkt)) > 0) {
        dbglog(DBG_DEBUG, "nsnd_poll_resp(): Received packet id %08lx, ts %08lx from AICA\n",
               pktcmd->cmd, pktcmd->timestamp);
    }

    if(rv < 0)
        dbglog(DBG_ERROR, "nsnd_poll_resp(): nsnd_aica_to_sh4 failed, giving up\n");
}
