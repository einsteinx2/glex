...
#ifndef __DC_SOUND_NSTREAM_H
#define __DC_SOUND_NSTREAM_H
...
#define NSTREAM_MAX 4

/** \brief  The maximum buffer size for a stream. */
#define NSND_STREAM_BUFFER_MAX 0x10000
...
*/
typedef void *(*nsnd_stream_callback_t)(nsnd_stream_hnd_t hnd, int smp_req,
                                       int *smp_recv);
...
/** \brief  Stream filter callback type.
    ...
    can be set with nsnd_stream_filter_add().
...
*/
typedef void (*nsnd_stream_filter_t)(nsnd_stream_hnd_t hnd, void *obj, int hz,
...
*/
void nsnd_stream_filter_add(nsnd_stream_hnd_t hnd, nsnd_stream_filter_t filtfunc,
                           void *obj);
...
/** \brief  Remove a filter from the specified stream.
    ...
    \param  obj             The filter function's user data. Must be the same as
                            what was passed as obj to nsnd_stream_filter_add().
*/
void nsnd_stream_filter_remove(nsnd_stream_hnd_t hnd,
                              nsnd_stream_filter_t filtfunc, void *obj);

/** \brief  Prefill the stream buffers.

    This function prefills the stream buffers before starting it. This is
    implicitly called by nsnd_stream_start(), so there's probably no good reason
...
/** \brief  Initialize the stream system.
    ...
    nsnd_init(), so it will potentially overwrite anything going on the AICA.
...
/** \brief  Shut down the stream system.
    ...
    with it. This does not call nsnd_shutdown().
*/
...
/** \brief  Enable queueing on a stream.
    ...
    that you must call nsnd_stream_queue_go() to actually start the stream, after
