...
#ifndef __DC_SOUND_NSOUND_H
#define __DC_SOUND_NSOUND_H
...
/** \brief  Free a block of allocated memory in the SPU RAM pool.
    ...
    This function frees memory previously allocated with nsnd_mem_malloc().
...
/** \brief  Get the size of the largest allocateable block in the SPU RAM pool.
    ...
    nsnd_mem_malloc() and expected to not return failure. There may be more
    memory available in the pool, especially if multiple blocks have been
    allocated and freed, but calls to nsnd_mem_malloc() for larger blocks will
...
/** \brief  Shutdown the SPU RAM allocator.
    ...
    calling nsnd_mem_init() afterwards.
*/
...
/** \brief  Initialize the sound system.
    ...
    nsnd_stream_drv will be loaded if a new program is uploaded to the SPU.
*/
int nsnd_init();
